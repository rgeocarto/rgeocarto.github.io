...
`tapply(X, INDEX, FUN)` permet d'agréger une variable en fonction d'une autre.
...
La fonction `st_buffer()` permet de construire des zones tampons. La distance est exprimée en unité du système de coordonnées utilisée (`st_crs(x)$units`).  
...
Plus d'infos : [Data first, nom d’un pipe](https://lcauquil.pages-forge.inrae.fr/mon_blog/posts/019_data_first_in_pipes/).  
:::
